-- Ngày thay đổi	: {CURRENT_TIME}
-- Người thay đổi	:
-- Phiên bản 		  :
-- Mô tả			    : Tạo stream multivalue

CREATE OR REPLACE STREAM FBNK_{TABLE_NAME}_MULTIVALUE WITH ( PARTITIONS=16, REPLICAS=3, RETENTION_MS=1814400000) AS SELECT
  DATA.ROWKEY ROWKEY,
  DATA.LOOKUP_KEY LOOKUP_KEY,
  DATA.RECID RECID,
  DATA.OP_TS OP_TS,
  DATA.CURRENT_TS REP_TS,
  TIMESTAMPTOSTRING(UNIX_TIMESTAMP(), 'yyyy-MM-dd HH:mm:ss.SSSSSS') CURRENT_TS,
  DATA.TABLE_NAME TABLE_NAME,
  DATA.COMMIT_SCN COMMIT_SCN,
  DATA.COMMIT_ACTION COMMIT_ACTION,
--{FIELD_NAMES},
  PARSE_T24_MULTIVAL(DATA.RECID, DATA.XMLRECORD, '{TABLE_NAME}_MULTIVALUE', ARRAY[''], ARRAY[''], '#') XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#
FROM FBNK_{TABLE_NAME}_MAPPED DATA
EMIT CHANGES;