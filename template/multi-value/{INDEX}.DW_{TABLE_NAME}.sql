...
CREATE OR REPLACE STREAM FBNK_{TABLE_NAME}_MULTIVALUE WITH (KAFKA_TOPIC='FBNK_{TABLE_NAME}_MULTIVALUE', PARTITIONS=1) AS SELECT
  DATA.ROWKEY ROWKEY,
  DATA.LOOKUP_KEY LOOKUP_KEY,
  DATA.RECID RECID,
  -- Để ý xem có phải bảng his hay không nếu có thêm trường recver (Note cho BNH: Sửa xong thì xoá) --
...
--{FIELD_NAMES},
  PARSE_T24_MULTIVAL(DATA.RECID, DATA.XMLRECORD, 'FBNK_{TABLE_NAME}_MULTIVALUE', ARRAY[''], ARRAY[''], '#') XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#; Tự điền các trường multi M , S ( note cho BNH : sửa xong thì xóa ) --
...
FROM FBNK_{TABLE_NAME}_MULTIVALUE DATA
EMIT CHANGES;